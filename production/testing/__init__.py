...
# Stolen from myself at
# https://github.com/Rigshospitalet-KFNM/DicomNode/blob/main/tests/helpers/dicomnode_test_case.py
class TransactionTracershopTestCase(TransactionTestCase):
  ...
  def assertRegexBefore(self,
    ...
    if isinstance(regex_1, str):
      regex_1 = compile(regex_1)

    if isinstance(regex_2, str):
      regex_2 = compile(regex_2)
...
class TracershopTestCase(TestCase):
  class __BeforeHandlerResponse(IntEnum):
    ORDERING_RESPECTED = 0 # success value
    REGEX_1_MISSING = 1
    REGEX_2_MISSING = 2
...
def clean_up_models(models: List[Type[Model]]):
  for ModelType in models:
    ModelType.objects.all().delete()