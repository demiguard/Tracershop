...
class DatabaseInterFaceTestCases(TracershopTestCase):
  ...
  def setUpTestData(cls):
    cls.test_admin = User.objects.create(username="test_admin",
  ...
  def test_createUserAssignment_existingUser(self):
    ...
    status, userAssignment, user = self.db.create_user_assignment(username, self.customer.id, self.test_admin)
  ...
  def test_createUserAssignment_MissingCustomer(self):
    username = "-AAAA0003"
    status, userAssignment, user = self.db.create_user_assignment(username, 189508160918, self.test_admin)
