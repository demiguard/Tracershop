...
import { LEGACY_KEYWORD_BID, LEGACY_KEYWORD_DELIVER_DATETIME, LEGACY_KEYWORD_RUN, LEGACY_KEYWORD_AMOUNT, LEGACY_KEYWORD_TRACER,
  WEBSOCKET_DATA, WEBSOCKET_DATATYPE, JSON_ACTIVITY_ORDER, JSON_CUSTOMER, PROP_ORDER_MAPPING, PROP_ON_CLOSE, PROP_WEBSOCKET, JSON_TRACER, PROP_ACTIVE_TRACER, JSON_ISOTOPE, WEBSOCKET_MESSAGE_MODEL_CREATE, PROP_ACTIVE_DATE, DATABASE_CURRENT_USER, AUTH_USER_ID, PROP_TIME_SLOT_ID, PROP_TIME_SLOT_MAPPING, JSON_ENDPOINT, JSON_DELIVER_TIME } from "../../lib/constants.js"
...
export function CreateOrderModal(props) {
  ...
  if(endpointMap === undefined){
    ...
    setState({
      ...
      if (1 == endpointOptions.length){
        endpointForm = <FormControl value={endpointOptions[0].name} readOnly/>
    }
      ...
      console.log("Code should not be called is called")
  ...
  } else if(state.error.missingTimeSlot == "") {
    const customer = props[JSON_CUSTOMER].get(state.customerID)
    const newError = {...state.error,
      missingTimeSlot : `Kunden ${customer.short_name} har ingen leveringtidspunkter til i dag.`
    }
    setState({
      error : newError,
    })
  ...
  const deliveryDateTime = combineDateAndTimeStamp(props[PROP_ACTIVE_DATE], currentTimeSlot.delivery_time)
