...
import { JSON_ACTIVITY_ORDER, JSON_ADDRESS, JSON_CLOSED_DATE, JSON_CUSTOMER, JSON_DATABASE, JSON_DELIVER_TIME, JSON_EMPLOYEE, JSON_INJECTION_ORDER, JSON_ISOTOPE, JSON_RUN, JSON_SERVER_CONFIG, JSON_TRACER, JSON_TRACER_MAPPING, JSON_VIAL, PROP_LOGOUT, PROP_NAVBAR_ELEMENTS, PROP_SET_USER, PROP_TRACERSHOP_SITE, PROP_USER, PROP_WEBSOCKET } from "./constants.js"
...
export function propsExtraction(props){
  ...
  if (!props.hasOwnProperty(JSON_CLOSED_DATE)) throw "missing JSON_CLOSED_DATE"
  if (!props.hasOwnProperty(JSON_CUSTOMER)) throw "missing JSON_CUSTOMER"
  if (!props.hasOwnProperty(JSON_DELIVER_TIME)) throw "missing JSON_DELIVER_TIME"
  ...
  if (props.hasOwnProperty(PROP_TRACERSHOP_SITE)){
    newProps[PROP_TRACERSHOP_SITE] = props[PROP_TRACERSHOP_SITE];
  }

  if (props.hasOwnProperty(PROP_SET_USER)){
    newProps[PROP_SET_USER] = props[PROP_SET_USER];
  }

  newProps[JSON_ADDRESS] = props[JSON_ADDRESS];
  newProps[JSON_DATABASE] = props[JSON_DATABASE];
  newProps[JSON_CLOSED_DATE] = props[JSON_CLOSED_DATE];
  newProps[JSON_CUSTOMER] = props[JSON_CUSTOMER];
  newProps[JSON_DELIVER_TIME] = props[JSON_DELIVER_TIME];
