...
} from '~/components/modals/activity_modal'
...
import { TracerCatalog, TracerCatalogProvider } from '~/contexts/tracer_catalog';
import { OrderMapping } from "~/lib/data_structures/order_mapping";
import { applyFilter, dailyActivityOrderFilter } from "~/lib/filters";
...
describe("Activity Modal Test", () => {
  it("Standard Render Test status 1", () => {
    ...
    const props = {
      [PROP_ACTIVE_TRACER] : 1,
      [PROP_ORDER_MAPPING] : new OrderMapping(toMapping(newOrders)),
      [PROP_TIME_SLOT_ID] : 1,
      [PROP_ON_CLOSE] : on_close_mock
    }

    render(
      <TracerShopContext tracershop_state={customState} websocket={websocket} dispatch={() => {}}>
  ...
  it("free an order at the date", async () => {
    ...
    props[PROP_ORDER_MAPPING] = new OrderMapping(toMapping(todays_orders));
  ...
  it("Update vial from state",() => {
    ...
    rerender(
      <TracerShopContext tracershop_state={newState} websocket={websocket} dispatch={() => {}}>
        <TracerCatalogProvider>
