...
describe("tracer websocket test suite", () => {
  ...
  it("Received websocket Get Orders", async () => {
    ...
    message[JSON_CLOSEDDATE] = [JSON.stringify(closedDate)]
    message[WEBSOCKET_DEAD_ORDERS] = [2]
    server.send(message);

    expect(stateHolder.UpdateMaps).toBeCalledWith(
      [DATABASE_ACTIVITY_ORDER, DATABASE_INJECTION_ORDER, DATABASE_VIAL, DATABASE_CLOSEDDATE],
